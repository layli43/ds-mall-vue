...
<script>
	...
	export default {
		...
		data() {
			return {
				cateMaskState: 0, // Category panel expansion state
				headerPosition: "fixed",
				headerTop: "0px",
				loadingType: 'more', // Load more status: more, loading, nomore
				filterIndex: 0,
				priceOrder: 0, // 1: Price Low to High, 2: Price High to Low
				cateList: [],
				productList: [],
				searchParam: {
		...
		onLoad(options) {
			// #ifdef H5
			// Calculate header offset for H5 compatibility
		...
		onPageScroll(e) {
			// Compatibility for iOS top drift during pull-down
			if (e.scrollTop >= 0) {
		...
		methods: {
			...
			// Load product data with pull-down refresh and infinite scroll
			async loadData(type = 'add', loading) {
				...
				if(this.filterIndex == 0){
					this.searchParam.sort = 0; // Recommended
				}
				if (this.filterIndex === 1) {
					this.searchParam.sort = 2; // Best Selling
				}
				if (this.filterIndex === 2) {
					// Price Sort
					if (this.priceOrder == 1) {
						this.searchParam.sort = 3; // Price Ascending
					} else {
						this.searchParam.sort = 4; // Price Descending
					}
				}
